...
class CommonFunction(BaseFunction):
    ...
    def eval(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        for joint_id in range(17):
            joint_acc_list.append(AverageMeter())
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                ...
                if self.PE_Name == 'DCPOSE':
                    ...
                    sup = input_sup_A if random_num <= 4 else input_sup_B
                    concat_input = torch.cat((input_x, sup), 1).cuda()
                ...
                if self.DataSetName == "PENNACTION":
                    box_xywh = meta['box_xywh']
                    origin_joints = meta['joints_origin'].detach().cpu().numpy()
                    origin_points = origin_joints[:, :, :2]
                    # total_acc, avg_acc, cnt, _, match_cnt, num_cnt = pck_accuracy(pred_heatmaps.detach().cpu().numpy(),
                    ...
                    total_acc, avg_acc, cnt, _, match_cnt, num_cnt = pck_accuracy_origin_image(preds,
                                                                                               origin_points,
                                                                                               box_xywh, thr=0.2)

                    for joint_id in range(17):
                        joint_acc = joint_acc_list[joint_id]
                        joint_acc.update(total_acc[joint_id + 1], num_cnt[joint_id])
                else:
                    _, avg_acc, cnt, _ = accuracy(pred_heatmaps.detach().cpu().numpy(), target_heatmaps.detach().cpu().numpy())
                acc.update(avg_acc, cnt)
                ...
                if iter_step % self.cfg.PRINT_FREQ == 0 or iter_step >= (num_batch - 1):
                    ...
                    # for seq_id, one_acc in enumerate(joint_acc_list):
                    #     logger.info("seq_id : {} ; {acc.val:.3f} ; {acc.avg:.3f}".format(seq_id, acc=one_acc))
        ...
        table_data = []
        for seq_id, one_acc in enumerate(joint_acc_list):
            table_data.append([seq_id, one_acc.avg])

        logger.info(tabulate(table_data))
